...
from .vietnam_news import get_global_news_vietnam, get_news_vietnam
...
def route_to_vendor(method: str, *args, **kwargs):
    ...
    # Vietnam symbols are a deliberate market-specific override: VNstock is the
    # primary price/fundamental source, while Vietnam news may fall back to Yahoo.
    vietnam_chain = _vietnam_primary_vendors(method, args)
    explicit = [vendor for vendor in primary_vendors if vendor and vendor != "default"]
    if vietnam_chain:
        vendor_chain = [
            vendor for vendor in vietnam_chain if vendor in VENDOR_METHODS[method]
        ]
    elif explicit:
        vendor_chain = [
    ...
    for vendor_index, vendor in enumerate(vendor_chain):
        ...
        try:
            ...
            if (
                method in ("get_news", "get_global_news")
                and _looks_like_empty_news(result)
                and vendor_index < len(vendor_chain) - 1
            ):
                continue
            return result
